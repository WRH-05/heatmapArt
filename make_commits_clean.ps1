# make_commits_clean.ps1 - Alternative version with cleaner file changes
...
foreach ($d in $dates) {
    # Option 1: Update a counter in the HTML (cleaner approach)
    $htmlContent = @"
<!doctype html>
<meta charset='utf-8'>
<title>CHROME art</title>

<body>
    <h1>CHROME</h1>
    <p>Commit #$counter on $d</p>
</body>
"@
    ...
    Write-Host "Created commit #$counter for $d"
    $counter++
}

Write-Host ""
Write-Host "Done creating commits!"
Write-Host "Final file shows: Commit #$($counter-1)"
